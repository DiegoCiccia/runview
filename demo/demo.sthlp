...
{vieweralsosee "[CAUSAL] Causal inference commands" "help causal_inference"}{...}
...
{vieweralsosee "[R] wildbootstrap" "help wildbootstrap"}{...}
...
{viewerjumpto "Video examples" "regress##video"}{...}
...
{syntab:SE/Robust}
{synopt :{opth vce:(regress##vcetype:vcetype)}}{it:vcetype} may be {opt ols},
   {opt r:obust}, {opt cl:uster} {it:clustvarlist}, {opt boot:strap}, 
   {opt jack:knife}, {opt hc2} [{it:clustvar}], or {opt hc3}{p_end}
...
{synopt :{opt clustert:able}}display table of multiway cluster combinations{p_end}
...
{p 4 6 2}
{cmd:bayes}, {cmd:bootstrap}, {cmd:by}, {cmd:collect}, {opt fmm}, {opt fp},
...
for intragroup correlation ({cmd:cluster} {it:clustvarlist}), and that use
...
{phang2}
{cmd:vce(cluster} {it:clustvarlist}{cmd:)} specifies that standard errors
allow for intragroup correlation within groups defined by one or more
variables in {it:clustvarlist}, relaxing the usual requirement that the
observations be independent.  For example, {cmd:vce(cluster clustvar1)}
produces cluster-robust standard errors that allow for observations that are
independent across groups defined by {cmd:clustvar1} but not necessarily
independent within groups.  You could also type {cmd:vce(cluster}
{cmd:clustvar1} {cmd:clustvar2} ... {cmd:cluster}{it:p}{cmd:)} to account for
correlation within groups formed by {it:p} variables (multiway clustering).

{pmore}
{cmd:regress} also allows the following:

{phang2}
{cmd:vce(hc2} [{it:clustvar}][{cmd:, dfadjust}]{cmd:)} and {cmd:vce(hc3)}
specify alternative bias corrections for the robust variance calculation.
{cmd:vce(hc2)} and {cmd:vce(hc3)} may not be specified with the {cmd:svy}
prefix.  In the unclustered case, {cmd:vce(robust)} uses
(sigma-hat_j)^2={c -(}n/(n-k){c )-} (u_j)^2 as an estimate of the variance of
the jth observation, where n is the number of observations, k is the number of
regressors, u_j is the calculated residual, and n/(n-k) is included to improve
the overall estimate's small-sample properties.
...
intervals.  {cmd:vce(hc2} {it:clustvar}{cmd:)} produces estimates that allow
for intragroup correlation within groups defined by {it:clustvar}.
{cmd:dfadjust} computes the 
{help regress##bellmccaffrey2002:Bell and McCaffrey (2002)} adjusted degrees
of freedom based on {it:clustvar}.  Note that {cmd:dfadjust} does not affect
multiple-imputation results when the command is used with {cmd:mi estimate}.
See {mansection R regressMethodsandformulas:{it:Methods and formulas}} in
{bf:[R] regress} for a description of the computation when {it:clustvar} is
specified.
...
{cmd:vce(cluster} {it:clustvarlist}{cmd:)} or the {cmd:svy} prefix.
...
defined in {helpb maximize:[R] Maximize} and that {it:string} be used to label
...
{phang}
{opt clustertable} displays a table reporting cluster combinations and the
number of clusters per combination.  This option is available only when
{cmd:vce(cluster} {it:clustvarlist}{cmd:)} is specified with more than one
variable in {it:clustvarlist} to compute multiway cluster-robust standard
errors.

INCLUDE help displayopts_listd

{phang2}
{opt dfci} specifies that parameter degrees of freedom and confidence
intervals be reported in the coefficient table.

{phang2}
{opt dfpvalues} specifies that parameter degrees of freedom and {it:p}-values
be reported in the coefficient table.
...
{bf:[R] regress}) and affecting calculated standard errors.  Degrees of
freedom for {it:t} statistics is calculated as n rather than n-k.
...
{phang}
{browse "https://youtu.be/_ti7Lju1odk":Fitting and interpreting regression models: Linear regression 
> with categorical predictors}

{phang}
{browse "https://youtu.be/D5Szv8SwJN4":Fitting and interpreting regression models: Linear regression 
> with continuous predictors}

{phang}
{browse "https://youtu.be/7f8dQfYoCG8":Fitting and interpreting regression models: Linear regression 
...
{synopt:{cmd:e(title)}}title in estimation output when {cmd:vce()} is not
             {cmd:ols}{p_end}
{synopt:{cmd:e(clustvar)}}names of cluster variables{p_end}
{synopt:{cmd:e(cluster}{it:#}{cmd:)}}cluster combination {it:#}{p_end}
...
{synopt:{cmd:e(beta)}}standardized coefficients{p_end}
{synopt:{cmd:e(V_modelbased)}}model-based variance{p_end}
{synopt:{cmd:e(adj_df)}}adjusted degrees of freedom when {cmd:vce(hc2, dfadjust)} is specified{p_end}
{synopt:{cmd:e(kcluster)}}cluster sizes, multiway clustering{p_end}
...
{browse "http://www.stata.com/bookstore/mhe.html":{it:Mostly Harmless Econometrics: An Empiricist's C
> ompanion}.}
Princeton, NJ: Princeton University Press.

{marker bellmccaffrey2002}{...}
{phang}
Bell, R. M., and D. F. McCaffrey. 2002. Bias reduction in standard errors for
linear regression with multi-stage samples.
{it:Survey Methodology} 28: 169-181.
